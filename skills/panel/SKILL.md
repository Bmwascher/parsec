...
- **The brief** follows `templates/brief-panel.md`, which holds what the host must give and the order the lanes answer in. One brief file serves every lane, each lane its own named background task.
...
- **Where it lives.** In the open feature folder, or, about no feature, under `setup`'s `panels\` folder, which the tool makes. Panel round folders are not committed while the panel is open (`setup`, "Tracked docs root").
