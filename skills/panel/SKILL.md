...
- **The brief** follows `templates/brief-panel.md`. One brief file serves every lane, each lane its own named background task.
...
- **Poll.** "Poll Fable" from an Opus session is not a round: dispatch `reviewer-fable` (`Fable Poll`) with the same kind of brief (a file in the open feature folder's `briefs\`, or a loose file under `panels\` when there is none) and no report path, and post its answer in chat. A same-vendor second opinion, never a gate, recorded only when Brandon asks.
