...
- Start the round with Monitor, `timeout_ms` 1800000 (Monitor kills the script at expiry; the tool's own cap is 28 minutes), named before launch from the driver rules' list: `round run` for a CLI lane (astra, sol, kimi; `--lane` omitted takes the config's `codex_lane`); `round prepare`, then the reviewer agent, then `round collect` for opus or fable. `round prepare` prints the package root, the code root and its commit, the report path and the task name for the dispatch.
