...
Run `build archive --feature <folder> --task N` before ANY redispatch of a task that already has a report, so no attempt's evidence is overwritten. The `implementer` reports blocked itself.
