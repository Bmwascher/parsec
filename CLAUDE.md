...
- **Skills name the tool by `${CLAUDE_PLUGIN_ROOT}/tools/parsec.py`**, never a bare path and never by searching (old item 58).
- **Budgets are tests.** `evals/tests/test_budget.py` counts the tool (1,250 lines), the tests (1,500), the frozen checkers (522) and the prose (11,000 words, 1,600 per skill). An addition that would pass a budget deletes something first.
