...
- **Print mode runs no command and has no delete tool** (2026-09-22, `gemini_probes.py`): a `RunCommand` was soft-denied; a task that deletes, renames or moves a file goes to the `backup-implementer`.
...
- **It never reports blocked** (2026-09-22; the account and the diagnosis order are in `build`, "A failing task").
- **`--add-dir` needs an absolute path** (field phase 5b, 2026-09-23): given `.`, agy logged `failed to resolve --add-dir path "."`, dropped the workspace and soft-denied its reads; `build run` resolves the checkout.
