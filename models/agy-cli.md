...
- **The trust list is not an authorization control** (old item 105): nothing mechanical refuses an `--add-dir` outside it. The checkout is the child's working folder and its `--add-dir`.
...
- **Gemini's tokens are recorded nowhere** the tool can read; a task's cost on this lane is time only (63 s on Task 2 of the comparison, 0 Claude tokens).
- **Auth is a silent refresh** seen only in the log (`silent auth succeeded`, or `not authenticated`); the first run's log is the login check.
- **The mode flag**: on 1.1.7 `--mode accept-edits` did not apply in print mode; on 1.2.0 without it the edit was soft-denied (2026-09-12). A `/c/...` log path produced no log file (2026-09-13).
