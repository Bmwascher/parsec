...
def feature_dir(cfg, primary, feature):
    ...
    if not fdir.is_dir():
        if rel.parts[:1] == ("panels",) and len(rel.parts) == 2:   # the one feature folder the tool makes itself
            ...
            raise Exit(64, f"feature folder {fdir} does not exist (a mistyped name must not grow a second docs root)")
...
def prepare(args, launch):
    ...
    if cli:
        ...
        program(row)                             # before any worktree is made
...
def codex_quota(prog):
    ...
    try:
        ...
        p = subprocess.Popen([prog, "app-server", "--stdio"], stdin=subprocess.PIPE, stdout=subprocess.PIPE,
...
def preflight(args):
    ...
    if args.lane in ("astra", "sol"):
        ...
        lines.append(f"{ver}   {login}   {codex_quota(shutil.which(cmd[0])) if code == 0 else ''}")
        ...
        alias = f'[models."{row["model"]}"]' in conf
        effort = "default_effort" in conf
        if not alias:
...
def main(argv=None):
    ...
    try:
        ...
        if args.cmd == "remove-worktree":
            remove_worktree(primary_of(args.repo), Path(args.path))
